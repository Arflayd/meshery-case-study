...
async function makeRequest () {
    ...
    const orderSize = parseInt(orderSizeRef.value);
    try {
        const data = await createOrder(orderSize);
        const endTime = new Date();
        historyRecords.push([historyRecords.length + 1, data.succes, data.fail, endTime.getTime() - startTime.getTime()])
    } catch (error) {
        const endTime = new Date();
        historyRecords.push([historyRecords.length + 1, 0, orderSize, endTime.getTime() - startTime.getTime()])
...
function initialize () {
    ...
    orderSizeRef = document.getElementById('order-size');
    rerenderTable()
}